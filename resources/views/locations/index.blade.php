...
@section('content')
    <div id="saved-locations" class="locations row">
        @forelse($locations as $key => $location)
            <div class="col-sm-6 col-md-4">
                <div id="location-{{ $location->id }}" class="location">
                    <button type="button" class="delete" data-for="#location-{{ $location->id }}" data-target="{{ route('locations::delete', [$location->id]) }}" data-token="{{ csrf_token() }}">
                        <i class="fa fa-trash-o"></i>
                    </button>
                    <div class="map" data-latitude="{!! $location->latitude !!}" data-longitude="{!! $location->longitude !!}">

                    </div>
                    <div class="address-label">{!! $location->first_address_line . ', ' . $location->postcode !!}</div>
                </div>
            </div>
        @empty
        @endforelse
